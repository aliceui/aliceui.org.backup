...
3. 使用 iconfont 和 CSS3 技术。
    ...
    而是使用了 iconfont 和渐进增强的 CSS3 技术，视觉上有更好的适应性和现代感，
    而在低端浏览器下只保证了基础的视觉支持（比如没有圆角）。

    不使用图片的另一个好处是可以在页面中大胆使用通用模块而不用担心请求数过多影响性能。

    > 注：一切必须用图片才能实现的视觉效果都是耍流氓。
...
Alice 基于支付宝的实际需求产出了一套通用样式模块，因此我们仍然需要支持 IE6/7 （是说样式模块比如 [button](http://aliceui.org/button/)，不是本站），
